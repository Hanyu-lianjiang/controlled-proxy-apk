...
elif [[ ! -x "$NDK_BUILD" && -x "$NDK_HOME/build/ndk-build" ]]; then
  NDK_BUILD="$NDK_HOME/build/ndk-build"
fi
if [[ ! -x "$NDK_BUILD" && ! -f "$NDK_BUILD" ]]; then
  echo "Android NDK: ndk-build not found under \$NDK_HOME"
  exit 1
fi
TMPDIR=$(mktemp -d)
clear_tmp () {
...
ln -s "$__dir/hev-socks5-tunnel" jni/hev-socks5-tunnel || cp -R "$__dir/hev-socks5-tunnel" jni/hev-socks5-tunnel
git -C "$__dir/hev-socks5-tunnel" rev-parse --short HEAD > .rev-id
git -C "$__dir/hev-socks5-tunnel" ls-files -s | while read -r mode _ _ path; do
  if [[ "$mode" == "120000" && ! -L "jni/hev-socks5-tunnel/$path" ]]; then
    target="$(cat "jni/hev-socks5-tunnel/$path")"
    target_path="$(cd "$(dirname "jni/hev-socks5-tunnel/$path")" && cd "$(dirname "$target")" && pwd)/$(basename "$target")"
    cp "$target_path" "jni/hev-socks5-tunnel/$path"
  fi
done
find jni/hev-socks5-tunnel -type f | while read -r file; do
  if [[ "$(wc -l < "$file")" == "0" ]]; then
    target="$(cat "$file")"
    target_path="$(cd "$(dirname "$file")" && cd "$(dirname "$target")" 2>/dev/null && pwd)/$(basename "$target")"
